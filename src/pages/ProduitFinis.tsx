...
const ProduitFinis = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/finished-products')
      .then(res => {
        if (!res.ok) throw new Error('Failed to fetch finished products');
        return res.json();
      })
      .then(data => {
        setProducts(data);
  ...
  useEffect(() => {
    // Fetch locations and build floorData with type property
    fetch('/api/locations')
      .then(res => res.json())
      .then(locs => {
    ...
    <Layout>
      <div className="mb-4 sm:mb-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold mb-1">Produits Finis</h1>
            <p className="text-sm sm:text-base text-muted-foreground">
              Suivi et gestion des produits finis
            </p>
          </div>
        </div>
      </div>
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableLoadingRow colSpan={8} text="Chargement des produits finis..." />
              ) : error ? (
                <TableRow>
                  ...
                  >
                    ...
                    <TableCell>
                      <button
                        ...
                        onClick={() => navigate(`/produitfinis/${product.reference.replace('REF-', '')}`)}
