...
import { generateQRCodeImage, generateQRCodeSVG, type QRCodeData } from '@/lib/qrcode';

interface ProductOperationDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  operationType: 'Entrée' | 'Sortie' | 'Complément Stock' | 'Transfer' | '';
...
const SemiOperationDialog: React.FC<ProductOperationDialogProps> = ({
  open,
  onOpenChange,
  operationType,
  ...
  movementHistory: propMovementHistory = [], // <-- Add default
  onDataChanged, // <-- Add this prop
}) => {
  // --- State ---
  ...
  const [sortieType, setSortieType] = useState<'local' | 'out'>('out');
  // Entrée
  ...
  const handleQRCodeScan = async (qrData: string) => {
    ...
    try {
      ...
      console.log('[DEBUG] Available locations:', filteredLocations);
  ...
  const handlePipeSeparatedQRCode = async (qrData: string) => {
    ...
    try {
      ...
      for (const [locationName, floors] of Object.entries(propFloorData)) {
  ...
  const handleTransferQRCodeScan = async (qrData: any) => {
    ...
    try {
      ...
      if (qrData.location && propLocations.includes(qrData.location)) {
  ...
  const handleLotNumberLookup = async (lotNumber: string) => {
    ...
    try {
      ...
      console.log('[DEBUG] handleLotNumberLookup - propMovementHistory length:', propMovementHistory?.length);
      ...
      if (!propMovementHistory || !Array.isArray(propMovementHistory)) {
      ...
      const productMovements = propMovementHistory.filter(m => m.product_id === product?.id);
  ...
  useEffect(() => {
    if (open && (operationType === 'Entrée' || operationType === 'Complément Stock' || operationType === 'Transfer')) {
  ...
  useEffect(() => {
    if (open) {
      console.log('[DEBUG] floorData:', propFloorData);
    }
  }, [open, propFloorData]);

  // --- Helpers ---
  const getAvailableFloors = (location: string) => {
    return propFloorData[location as keyof typeof propFloorData] || [];
  };
  ...
  const generateQRCode = async () => {
    try {
      ...
      const qrData: QRCodeData = {
        ...
        productName: product?.designation || product?.nom || product?.name || 'N/A',
        productReference: product?.reference || product?.ref || 'N/A',
        batchNumber: operationForm.batchNumber || generateBatchNumber(),
        ...
        floors: selectedZones.flatMap(zone => 
          zone.floors.filter(f => f.quantity > 0).map(f => ({
            name: f.floorName,
            quantity: f.quantity
          }))
        ),
        operationType: operationType + ' (Semi-fini)',
        timestamp: new Date().toISOString()
      };

      const qrCodeImage = await generateQRCodeImage(qrData, false); // Use complex format for semi-operation QR codes
          ...
          <div class="print-content">
            ...
            <div class="header-section">
              <div class="header-flex">
                ...
                <div class="title-section">
                  ...
                  <h2 class="sub-title">PRODUIT SEMI-FINI RETOURNÉ EN STOCK</h2>
            ...
            <div class="content-section">
              ...
              <div class="left-section">
                <h3 class="section-title">Informations du produit semi-fini</h3>
              ...
              <div class="right-section">
                ...
                <div class="field-group">
                  ...
                  <div class="field-value">Complément Stock - Retour partiel (Semi-fini)</div>
                </div>
            ...
            <div class="footer-section">
              <p class="footer-text">
                <strong>Note importante :</strong> Ce produit semi-fini est un complément de stock - il s'agit d'un produit qui n'a pas été utilisé entièrement
  ...
  const handleLocationChange = (location: string) => {
    ...
    if ((operationType === 'Entrée' || operationType === 'Complément Stock') && location) {
  ...
  function computeFifoStock() {
    ...
    const entries = propMovementHistory.filter(m => m.status === 'Entrée');
    const sorties = propMovementHistory.filter(m => m.status === 'Sortie');
  ...
  if (operationType === 'Sortie' && Array.isArray(propMovementHistory)) {
    // Group by lot, location, etage, part
    const groupMap: Record<string, any> = {};
    for (const row of propMovementHistory.filter(row => row.product_id === product?.id && row.is_transfer !== 1 && row.is_transfer !== '1' && row.is_transfer !== true && row.internal_transfer !== 1 && row.internal_transfer !== '1' && row.internal_transfer !== true)) {
      const key = [
  ...
  const calculateAvailableStock = (location: string, lotBatchNumber: string) => {
    if (!lotBatchNumber || !Array.isArray(propMovementHistory) || !product?.id || !location) {
      return [];
    }

    // Group movements by location, etage/part for this specific lot
    const groupMap: Record<string, any> = {};

    for (const row of propMovementHistory.filter(row =>
      row.product_id === product?.id &&
      (row.batch_number === lotBatchNumber || row.lot === lotBatchNumber) &&
      row.location_name === location &&
      // Exclude transfer movements from stock calculation
      row.is_transfer !== 1 && row.is_transfer !== '1' && row.is_transfer !== true &&
      row.internal_transfer !== 1 && row.internal_transfer !== '1' && row.internal_transfer !== true
    )) {
      const etageId = row.etage_id || null;
      const partId = row.part_id || null;
      const key = `${location}-${etageId}-${partId}`;
    ...
    // Convert to available floors format
    const availableFloors = [];
    for (const [key, data] of Object.entries(groupMap)) {
      const available = data.entree - data.sortie;
      if (available > 0) {
        const floorName = data.etageId ? data.etage_name : data.part_name;
        const floorId = data.etageId || data.partId;
        const type = data.etageId ? 'etage' : 'part';
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const isEntry = operationType === 'Entrée' || operationType === 'Complément Stock';
    ...
    const product_type = 'semi'; // Always use 'semi' for this dialog
    const fournisseur_id = isEntry ? parseInt(operationForm.fournisseur) : undefined;
    // Dates are only used for entry operations (Entrée, Complément Stock, Transfer)
    // For sortie operations, dates come from the original entry movements
    const fabricationDate = isEntry ? (operationForm.fabricationDate || new Date().toISOString().slice(0, 10)) : '';
    const expirationDate = isEntry ? (operationForm.expirationDate || null) : ''; // Allow null for expiration date
    ...
    // For "Prêt à l'emploi" mode, supplier is required (when mode is undefined, supplier is optional)
    if (operationType === 'Entrée' && operationMode === 'ready' && !fournisseur_id) {
      toast.error('Veuillez sélectionner un fournisseur pour les produits prêts à l\'emploi.');
      setIsSaving(false);
      return;
    }
    try {
      if (operationType === 'Entrée' || operationType === 'Complément Stock') {
        const payloads = allSelectedFloors.map(floor => ({
          product_type, // use 'semi'
          product_id,
          status,
          quantity: floor.quantity,
          location_id: locationIdMap[floor.zone],
          fournisseur_id: (isComplementStock || (operationType === 'Entrée' && (operationMode === 'ready' || operationMode === undefined) && fournisseur_id)) ? fournisseur_id : undefined, // For Complément Stock and when supplier is provided
          fabricationDate,
          expirationDate,
        ...
        let visibleRows = [];
        if (Array.isArray(propMovementHistory)) {
      ...
      // Generate QR code for entry operations (Entrée, Complément Stock)
      if (isEntry) {
        await generateQRCode();
      }

      if (onDataChanged) onDataChanged(); // Call refresh after success
      onOpenChange(false);
      onSuccess({
        type: operationType,
        ...operationForm,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="space-y-4">
                  ...
                  <div className="space-y-4 p-4 border rounded-lg">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => {
                          console.log('[DEBUG] Transfer Scanner button clicked');
                          setShowQrScanner(true);
                          setScannerLoading(false);
                          setManualJsonInput('');
                        }}
                        disabled={isLotLookupLoading}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          {propLocations.filter(loc => loc !== transferSourceLocation).map(location => (
                            <SelectItem key={location} value={location}>{location}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                          ...
                          <div className="space-y-2 max-h-40 overflow-y-auto">
                            {transferDestinationFloors.map(floor => (
                              <div key={floor.floorId} className="flex items-center gap-2 p-2 border rounded bg-gray-50">
                                ...
                                <span className="text-sm font-medium">{floor.quantity} {product?.unite || 'unités'}</span>
                              </div>
                            ))}
                          </div>
          ...
          ) : (operationType === 'Entrée' || operationType === 'Complément Stock') ? (
            <>
              ...
              {(operationType !== 'Complément Stock' || lotFound) && (
                <>
                  ...
                  {/* Fournisseur select - show for all Entrée operations when mode is undefined (fallback) */}
                  {operationType === 'Entrée' && (operationMode === 'ready' || operationMode === undefined) && (
                    <Select
                      value={operationForm.fournisseur}
                      onValueChange={val => setOperationForm(f => ({ ...f, fournisseur: val }))}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={operationMode === undefined ? "Sélectionner un fournisseur (optionnel)" : "Sélectionner un fournisseur"} />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  {operationType === 'Entrée' && (operationMode === 'local' || operationMode === 'ready') && (
            ...
            <>
              <div className="flex flex-col gap-4 py-2">
                <div className="flex gap-4">
                  <Button
                    ...
                    className={sortieType === 'local' ? 'bg-black text-white hover:bg-gray-800' : 'opacity-80'}
                  >
                    Transfert interne
                  </Button>
                  <Button
                    ...
                    className={sortieType === 'out' ? 'bg-black text-white hover:bg-gray-800' : 'opacity-80'}
                  >
                    Sortie externe
                  </Button>
                </div>
              ...
              {operationForm.quantity && parseInt(operationForm.quantity) > 0 && operationType === 'Sortie' && (
                <div className="mb-4">
                  <div className="space-y-3">
                    {(() => {
                      if (!Array.isArray(propMovementHistory) || propMovementHistory.length === 0) {
                      ...
                      const filteredMovements = propMovementHistory.filter(row =>
                        row.product_id === product?.id &&
                        row.is_transfer !== 1 &&
                        row.is_transfer !== '1' &&
          ...
          <Button
            disabled={
              ...
              operationType === 'Nouveau Produit'
                ...
                // Regular Entrée: Different validation based on mode
                : operationType === 'Entrée'
                  ? !operationForm.quantity || !operationForm.location || selectedFloors.length === 0 || totalSelectedQuantity !== parseInt(operationForm.quantity) || !operationForm.fabricationDate || (operationMode === 'ready' && !operationForm.fournisseur) || (operationForm.batchNumber && operationForm.batchNumber.startsWith('LOT-') && operationForm.batchNumber.trim().length === 4)
                  // Sortie: Validation - check if quantity is available
                  : !operationForm.quantity || (operationType === 'Sortie' && (
                      !Array.isArray(propMovementHistory) || 
                      propMovementHistory.length === 0 || 
                      (propMovementHistory.reduce((available, row) => {
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="text-center text-sm text-gray-600">
              ...
              <p>de l'opération effectuée (Semi-fini)</p>
            </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (qrCodeData) {
                    ...
                    link.download = `qr-code-semi-${operationType}-${Date.now()}.png`;
                  ...
                  if (qrCodeData) {
                    ...
                    if (printWindow) {
                      printWindow.document.write(`
                        <html>
                          <head>
                            <title>Code QR - ${operationType} (Semi-fini)</title>
                            <style>
                              body { 
                                display: flex; 
                          ...
                          <body>
                            <div class="qr-container">
                              <div class="qr-info">
                                <h2>${operationType} (Semi-fini) - ${product?.designation || product?.nom || product?.name || 'Produit'}</h2>
                                <p>Lot: ${operationForm.batchNumber}</p>
...
export default SemiOperationDialog; 